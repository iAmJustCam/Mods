...
class DataFetcher:
    ...
    async def fetch_data(self, session: ClientSession, date: str) -> List[Dict]:
        retries = 3
    ...
    def invalidate_stale_cache(self):
...
class Backtester:
    ...
    async def execute_backtest(self):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [self.data_fetcher.fetch_data(session, date) for date in date_values]
            ...
            for date, daily_schedule in zip(date_values, results):
                ...
                self.data_fetcher.cache[date] = cleaned_data
                self.data_fetcher.cache_dates.add(date)
                schedule_data.extend(cleaned_data)

        self.data_fetcher.invalidate_stale_cache()
